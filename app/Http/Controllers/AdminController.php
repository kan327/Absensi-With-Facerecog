...
class AdminController extends Controller
{
    ...
    public function search_guru(Request $request)
    {
        ...
        if($jumlah_data < 1){
            ...
            <div class="absolute left-1/2 top-1/2 flex flex-col items-center" style="transform: translate(-50%, -50%);">
                            ...
                            <p class="font-semibold text-placeholder">Pencarian Tidak Ditemukan! <span class="text-bg-blue-dark font-bold" id="search_again"> Cari Lagi</span></p>
                        </div>
            ';
        }else{
    ...
    {
        $keyword = $request->search_jadwal;

        $data_mapel = mapel::where("pelajaran", "like", "".$keyword."%")->get();
        
        if($keyword != ''){

            $data = JadwalAbsen::with(['kelas', 'guru','mapel'])->whereIn("mapel_id", $data_mapel->pluck("id"))->orWhere("tanggal", "like", "".$keyword."%")->get();

        }else{

            $data = JadwalAbsen::with(['kelas', 'guru','mapel'])->get();
        ...
        if($jumlah_data < 1){
            ...
            return view("admin.component.table_jadwal", [
                "data_jadwal"=>$data,
            ]);
