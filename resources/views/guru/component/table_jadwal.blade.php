@foreach ($jadwal_absens as $jadwal_absen)
    <tr name="baris_jadwal" class="hover:bg-[#F5F5F5] hover:font-bold rounded-full in-hover-to">
        ...
        <td class="" style="border-top-right-radius: 12px; border-bottom-right-radius: 12px;">
            ...
            <a
                onclick="return Noticme.any({text: 'Hapus!', type: 'info', message: 'Apakah Anda Yakin Ingin Menghapus Data Berikut?', confirm:true}).then(result => { if(result){ location.href='/absensi/hapus/{{ $jadwal_absen->id }}/{{ $jadwal_absen->tanggal }}/{{ $jadwal_absen->kelas_id }}/{{ $jadwal_absen->mapel_id }}' } })">
                <i class="fa-regular fa-trash-can this-one p-2"></i>
                {{-- <span
                    class="material-symbols-outlined this-one">delete
                </span> --}}
            </a>
            <a @if ($time_now >= $jadwal_absen->selesai && $jadwal_absen->tanggal <= $date_now) href="/absensi/excel/{{ $jadwal_absen->tanggal }}/{{ $jadwal_absen->kelas_id }}/{{ $jadwal_absen->mapel_id }}" @else onclick="validate('Sesi Absen Belum Berakhir!')" @endif
                name="download_excel">
                <i class="fa-solid fa-download p-2"></i>
